...
async function generateUserMessage() {
  try {
    ...
    return `Answer this question very shortly and briefly: ${completion.choices[0].message.content}`;
  } catch (error) {
    log(`Error generating message with Groq: ${error.message}`, "error");
    return "Answer this question very shortly and briefly: Hi there, can you tell me something interesting?";
  }
}
